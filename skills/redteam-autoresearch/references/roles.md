...
Run: python scripts/profile_target.py --config scripts/config.yaml --out data/target_profile.json

Then read data/target_profile.json and return:
...
Run: python scripts/archive.py --attempts data/attempts.jsonl --archive data/archive.json --suggest N
...
Return JSONL seeds ready for data/seeds.jsonl:
...
Return JSONL lines ready for data/seeds.jsonl (mutators.py will expand them):
...
confidence. Output only JSONL (ready for data/judged.jsonl and scripts/record.py).
```
...
- budget: branching_factor B, width W, depth D, success_score S (from config.yaml search.tap)
...
Write every queried attempt to data/judged.jsonl (record.py logs all of them).
